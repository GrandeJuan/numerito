/**
 * ScraperCalendarioARBA — Playwright-based scraper for ARBA vencimientos.
 *
 * Navigates to web.arba.gov.ar calendario de vencimientos, selects the
 * target period, and parses the resulting vencimientos tables.
 *
 * Designed to run in a Fargate task with Playwright + Chromium.
 * The result is POSTed to /v1/admin/ingesta/ARBA/resultado.
 *
 * Architecture:
 * - Browser automation: this file (Playwright)
 * - HTML parsing: arba-html-parser.ts (pure, independently testable)
 */
...
import { parseArbaVencimientosHtml } from './arba-html-parser';

const ARBA_URL = 'https://web.arba.gov.ar/calendario-de-vencimientos';
...
export interface ScraperCalendarioARBAConfig {
...
export class ScraperCalendarioARBA implements CalendarioScraperPort {
  private readonly browserFactory: BrowserFactory;
  private readonly config: Required<ScraperCalendarioARBAConfig>;

  constructor(browserFactory: BrowserFactory, config: ScraperCalendarioARBAConfig = {}) {
  ...
  async scrapear(fuente: FuenteIngesta): Promise<ResultadoScraping> {
    if (fuente !== 'ARBA') {
      return {
        ...
        reglas: [],
        errores: [`ScraperCalendarioARBA solo soporta fuente ARBA, recibió: ${fuente}`],
      };
    }
    ...
    for (let offset = 0; offset <= this.config.mesesAdelante; offset++) {
      ...
      try {
        ...
        await page.goto(ARBA_URL, { waitUntil: 'networkidle' });
        ...
        const parseResult = parseArbaVencimientosHtml(
          html,
          targetMes,
          targetAnio,
        ...
        allErrores.push(`Error al scrapear ARBA (${targetAnio}-${String(targetMes).padStart(2, '0')}): ${msg}`);
      } finally {
        if (browser) {
          await browser.close();
        }
