...
import { ScraperCalendarioARBA } from './scraper-calendario-arba';
...
describe('ScraperCalendarioARBA', () => {
  it('should extract reglas from fixture HTML', async () => {
    ...
    const iibbT0 = result.reglas.find(
      (r) => r.tipoObligacion === TIPO_OBLIGACION.IIBB_ARBA && r.terminacionCuit === '0',
    );
  ...
  it('should reject wrong fuente', async () => {
    ...
    expect(result.reglas).toHaveLength(0);
    expect(result.errores[0]).toContain('solo soporta fuente ARBA');
  });
});
