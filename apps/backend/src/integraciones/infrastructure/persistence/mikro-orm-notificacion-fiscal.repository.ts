...
export class MikroOrmNotificacionFiscalRepository implements NotificacionFiscalRepository {
  ...
  async findByClienteId(clienteId: string, tenantId: string): Promise<NotificacionFiscal[]> {
  ...
  async findByTenantId(tenantId: string): Promise<NotificacionFiscal[]> {
    const entities = await this.em.find(NotificacionFiscalEntity, { tenant: { id: tenantId } }, {
      populate: ['cliente', 'tenant', 'organismo'],
    });
    return entities.map(e => this.toDomain(e));
  }
  ...
  async save(notificacion: NotificacionFiscal): Promise<void> {
    ...
    const tenant = this.em.getReference(EstudioEntity, notificacion.tenantId);
