import { EstudiosAdminListView } from './estudios-admin-list.view';

describe('EstudiosAdminListView', () => {
  let view: EstudiosAdminListView;
  let mockEm: any;
  ...
  beforeEach(() => {
    ...
    view = new EstudiosAdminListView(mockEm);
  });
  ...
  it('should apply search filter with $or on nombre and cuit', async () => {
    await view.execute({ search: 'demo' });
  ...
  it('should apply plan filter', async () => {
    await view.execute({ plan: 'PROFESIONAL' });
  ...
  it('should apply date range filters', async () => {
    await view.execute({ from: '2026-01-01', to: '2026-12-31' });
  ...
  it('should respect page and limit', async () => {
    await view.execute({ page: 3, limit: 5 });
